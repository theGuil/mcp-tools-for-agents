...
vídeo, áudio e arquivos. Sem API, sem UI, sem Python, sem `uv`. É a mesma
arquitetura e as mesmas 34 tools da versão Python do projeto, compiladas em um
executável único: baixe o `mcp-tools` da sua plataforma, aponte o host MCP para
ele e pronto.

Funciona com Claude Code, Claude Desktop, Cursor e qualquer host MCP.

## Por que uma versão em Rust

A versão Python precisa de Python 3.14, `uv`, FFmpeg no PATH e, para os extras,
de `faster-whisper` e OpenCV. A versão Rust precisa de **um arquivo**:
...
Baixe o pacote da sua plataforma na página de releases (ou compile, abaixo),
descompacte e rode:
...
Desktop) ou no equivalente do seu host. O `.mcp.json` desta pasta já aponta para
`./target/release/mcp-tools`. Pronto: peça "corta os 10 primeiros segundos do
intro.mp4" e veja acontecer.

## Tools

As mesmas da versão Python, com os mesmos nomes, parâmetros e retornos.
...
tentativas, nesta ordem, iguais à versão Python:
...
Todas as variáveis estão em `.env.example`. As novas em relação ao Python:
...
A árvore é a mesma da versão Python, com `lib.rs` e `main.rs` no lugar de
`__main__.py` (o `Cargo.toml` aponta para a raiz em vez de `src/`):
...
embaixo. `ToolResult` é o `@guarded` do Python: `Err(ToolError)` vira
`{error, code, hint}` na resposta.
