...
use mcp_tools::domains::video::remove_silence::remove_silence;
...
fn test_remove_silence_invalid_threshold() {
    ...
    let rt = runtime();
    let sample = sample_video(&rt.runtime.workspace);
    let error = remove_silence(&rt.runtime, &sample, 5.0, MIN_SILENCE, MARGIN, false).unwrap_err();
...
fn test_remove_silence_invalid_min_silence() {
    ...
    let rt = runtime();
    let sample = sample_video(&rt.runtime.workspace);
    let error = remove_silence(&rt.runtime, &sample, THRESHOLD_DB, 0.0, MARGIN, false).unwrap_err();
    assert_eq!(error.code, ErrorCode::InvalidArgument);
}
